...
parser.add_argument('--model_type', type=str, default='h_vnet', help='model_type')
...
from networks.hierarchical_vnet import VNet as HVNet
from networks.hierarchical_unet_3d import UNet_3D as HUNet_3D
...
if __name__ == "__main__":
    ...
    def create_model(ema=False):
        ...
        elif args.model_type == "h_vnet":
            net = HVNet(n_channels=1, n_classes=num_classes + 1, normalization='batchnorm', has_dropout=True, pyramid_has_dropout=True)
        elif args.model_type == "unet_3d":
            net = UNet_3D(in_channels=1, n_classes=num_classes + 1)
        elif args.model_type == "hunet_3d":
            net = HUNet_3D(in_channels=1, n_classes=num_classes + 1)
        model = net.cuda()
        if ema:
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if args.model_type == "vnet" or args.model_type == "unet_3d":
                outputs = model_seg(volume_batch)
            else:
            ...
            with torch.no_grad():
                if args.model_type == "vnet" or args.model_type == "unet_3d":
                    outputs_ema = model_seg_ema(volume_batch_ema)
                else:
            ...
            for param_group in optimizer_seg.param_groups:
                param_group['lr'] = lr_
            # print(optimizer_seg.param_groups[0]['lr'])
